...
#' Calcul de moyenne par application des méthodes issues des plans de sondage
#'
#' @param
#'
#' @reference guide lcsqa
estimation_sondage <- function(echantillon, auxiliaire, redressement=NULL, premier=T, iso=NULL, grappe='grappe', strate='strate')
{
	...
	if(!is.null(redressement))
	{
		...
		grappe_d <- tapply(start (echValide), echValide[['grappe']], min)
		grappe_f <- tapply(end (echValide), echValide$grappe, max)
	...
	if("diff" %in% redressement)
	{
		...
		{
			pop$m_diff <- sum(strate$poids*(strate$m_ech-strate$m_aux+strate$m_reelle))
			pop$v_diff <- sum(strate$poids^2*(strate$v_ech+strate$v_aux-2*strate$cov))
		} else {
			pop$m_diff <- NA
			pop$v_diff <- NA
		}
	...
	if("quot" %in% redressement) 
	{
		if(all(!is.na(strate$cov))) 
		{
			r <- pop$moy/sum(strate$poids*strate$m_aux)
			pop$m_quot <- sum(strate$poids*(strate$m_ech/strate$m_aux*strate$m_reelle))
			pop$v_quot <- sum(strate$poids^2*(strate$v_ech+r^2*strate$v_aux-2*r*strate$cov))
		} else {
			pop$m_quot <- NA
			pop$v_quot <- NA
		}
	...
	if("reg" %in% redressement)
	{
		if(all(!is.na(strate$cov)))
		{
			b <- strate$cov/strate$v_aux
			pop$m_reg <- sum(strate$poids*(strate$m_ech+b*(strate$m_reelle-strate$m_aux)))
			roCarre <- strate$cov^2/(strate$v_ech*strate$v_aux)
			pop$v_reg <- sum(strate$poids^2*strate$v_ech*(1-roCarre))
		} else {
			pop$m_reg <- NA
			pop$v_reg <- NA
		}
	...
	if(premier)
	{
		estimation <- data.frame(cbind(t(pop),
					       estimation_sondage(echantillon[-5],
								  if(length(auxiliaire)<=4)
									  auxiliaire else
									  auxiliaire[-4],
								  redressement, premier=F, iso)))
		names(estimation) <- names(echantillon)[-(1:4)]
		return(estimation)
	} else {
		return(cbind(t(pop), estimation_sondage(echantillon[-5],
							if(length(auxiliaire)<=4)
								auxiliaire else
								auxiliaire[-4],
							redressement, premier=F, iso)))
