...
#	if(length(unique(jours)) < 28)stop("pas suffisamment de donnees pour realiser un profil mensuel")
	...
	temp <- profil
	profil <- as.data.frame(matrix(NA, nrow=31, ncol=length(temp)))
...
#	attributes(profil)$lamberty <- attributesTemp$lamberty
	...
	class(profil) <- c("pmois", "Qair", "data.frame")
	profil
	}
